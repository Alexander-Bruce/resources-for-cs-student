...
module cpu(data_in,clk_quick,clk_slow,clk_delay,rst,SW_choose,A1,cpustate,addr,data_out,r0dbus,r1dbus,r2dbus,r3dbus,clr,irout,
				arload,arinc,pcload,pcinc,pcbus,drload,drhbus,drlbus,irload,trload,trbus,r0load,r0bus,r1load,r1bus,
				r2load,r2bus,r3load,r3bus,xload,yload,ybus,zload,read,write,membus,busmem,zout);
...
output clr,arload,arinc,pcload,pcinc,drload,trload,irload,r0load,r1load,r2load,r3load,xload,yload,
		zload,pcbus,trbus,r0bus,r1bus,r2bus,r3bus,ybus,read,write,membus,busmem,drhbus,drlbus;
...
y my(.din(aluout),.clk(clk_choose),.rst(rst),.yload(yload), .dout(ydbus));
//alu(alus,x, bus, dout, zout);补充alu实例化语句
alu malu(.alus(alus),.x(xout),.bus(dbus),.dout(aluout),.zout(aluzout));
//z(din,clk,rst,zload,dout);补充z实例化语句，如果需要的话
z mz(.din(aluzout),.clk(clk_choose),.rst(rst),.zload(zload),.dout(zout));
//control(din,clk,rst,z,cpustate,......,clr);补充control实例化语句
control mcontrol(.din(irout),.clk(clk_run),.rst(rst),.z(zout),.cpustate(cpustate),.clr(clr),	
						.arload(arload),.arinc(arinc),.pcload(pcload),.pcinc(pcinc),.pcbus(pcbus),.drload(drload),.drhbus(drhbus),
						.drlbus(drlbus),.irload(irload),.trload(trload),.trbus(trbus),.r0load(r0load),.r0bus(r0bus),.r1load(r1load),.r1bus(r1bus),
						.r2load(r2load),.r2bus(r2bus),.r3load(r3load),.r3bus(r3bus),.xload(xload),.alus(alus),.yload(yload),.ybus(ybus),
						.zload(zload),.read(read),.write(write),.membus(membus),.busmem(busmem));
