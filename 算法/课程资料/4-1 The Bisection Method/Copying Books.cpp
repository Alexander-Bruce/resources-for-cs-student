...
void print(long long ans) {
  ...
  int remain = k;
  for(int i = m-1; i >= 0; i--) {   //  minimize the work assigned to the first scriber ...
  // cannot be assigned any more or to ensure each scriber be assigned at least one book
...
int main() {
  ...
  while(T--) {
    ...
    while(L < R) { // to compute the value (the maximum sum of a single part should be as small as possible)
      long long M = L + (R-L)/2;
