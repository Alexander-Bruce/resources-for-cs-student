...
input clk_delay;
input clr;//其值来源于control.v模块，当clr有效时，时钟clk_run输出为0
input rst;
input SW_choose;//选择快慢时钟；1位，为1是快时钟，否则是慢时钟；top模块的输入，即SW15的状态
input A1;//按钮,按下为0，弹起为1；top模块的输入
input [1:0] cpustate;//CPU的状态，CPU_Controller模块的输出
output clk_run,clk_choose;//送入控制器control的时钟;寄存器ar、pc等的时钟
